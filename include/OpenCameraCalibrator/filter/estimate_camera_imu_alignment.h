...
// alignment stuff
using QuatVector =
    std::vector<Quaterniond, Eigen::aligned_allocator<Quaterniond>>;
using Vec3Vector = std::vector<Vector3d, Eigen::aligned_allocator<Vector3d>>;
using QuatMap = std::map<double, Quaterniond, std::less<double>,
                         Eigen::aligned_allocator<Quaterniond>>;
using Vec3Map = std::map<double, Vector3d, std::less<double>,
...
void EstimateCameraImuAlignment(
    const QuatMap &visual_rotations, const Vec3Map &imu_angular_vel,
