...
static const Eigen::Matrix4d M =
    (Eigen::Matrix4d() << 1. / 6., 4. / 6., 1. / 6., 0, -3. / 6., 0, 3. / 6., 0,
     3. / 6., -6. / 6, 3. / 6., 0, -1. / 6., 3. / 6., -3. / 6., 1. / 6.)
        .finished();

static const Eigen::Matrix4d M_cumul =
    (Eigen::Matrix4d() << 6. / 6., 5. / 6., 1. / 6., 0, 0. / 6., 3. / 6.,
     3. / 6., 0, 0. / 6., -3. / 6., 3. / 6., 0, 0. / 6., 1. / 6., -2. / 6.,
     1. / 6.)
        .finished();
...
struct SplineSegmentMeta : public entity::MetaData {
  ...
  SplineSegmentMeta(double dt, double t0) : dt(dt), t0(t0), n(0){};

  SplineSegmentMeta() : SplineSegmentMeta(1.0, 0.0){};
  ...
  double MinTime() const {
    if (segments.size() == 1)
      ...
      throw std::runtime_error(
          "Concrete splines must have exactly one segment");
  }
...
template <typename T, typename _ControlPointInfo>
class SplineSegmentView : public TrajectoryView<T, SplineSegmentMeta> {
 public:
  ...
  T t0() const { return T(this->meta_.t0); }

  T dt() const { return T(this->meta_.dt); }

  size_t NumKnots() const { return this->meta_.n; }

  double MinTime() const override { return this->meta_.MinTime(); }

  double MaxTime() const override { return this->meta_.MaxTime(); }
...
template <typename T, typename MetaType,
          template <typename...> typename SegmentTemplate>
 ...
 public:
  ...
  SplineView(const MetaType &meta, entity::ParameterStore<T> *holder)
    ...
    for (auto &segment_meta : meta.segments) {
      ...
      segments.push_back(std::make_shared<SegmentView>(
          segment_meta, holder->Slice(offset, length)));
      offset += length;
    }
  }
  ...
  double t0() const { return ConcreteSegmentViewOrError()->t0(); }

  double dt() const { return ConcreteSegmentViewOrError()->dt(); }

  int NumKnots() const { return ConcreteSegmentViewOrError()->NumKnots(); }

  void CalculateIndexAndInterpolationAmount(T t, int &i0, T &u) const {
    return this->ConcreteSegmentViewOrError()
        ->CalculateIndexAndInterpolationAmount(t, i0, u);
  }
...
struct SplineFactory {
  ...
  struct SegmentEntity
      : public TrajectoryEntity<SegmentView, SplineSegmentMeta,
                                entity::DynamicParameterStore<double>> {
    using Base = TrajectoryEntity<SegmentView, SplineSegmentMeta,
                                  entity::DynamicParameterStore<double>>;
    ...
    void AddToProblem(
        ceres::Problem &problem, time_init_t times, SplineSegmentMeta &meta,
        std::vector<entity::ParameterInfo<double>> &parameters) const override {
      throw std::runtime_error(
          "Segments do not know how to add to problem. This is handled by the "
          "Spline entity");
    }
    ...
    void AppendKnot(const ControlPointType &cp) {
      ...
      auto i = this->pstore_->AddParameter(
          this->control_point_info_.size,
...
template <template <typename> typename SegmentViewTemplate>
class SplineEntity
    : public TrajectoryEntity<SplineFactory<SegmentViewTemplate>::template View,
                              SplineMeta, _SplineParamStore> {
  using Base =
      TrajectoryEntity<SplineFactory<SegmentViewTemplate>::template View,
                       SplineMeta, _SplineParamStore>;
  using ControlPointType = typename Base::ControlPointType;
  using SegmentType =
      typename SplineFactory<SegmentViewTemplate>::SegmentEntity;

  // Hidden constructor, not intended for user code
  SplineEntity(double dt, double t0,
               ceres::LocalParameterization *control_point_parameterization)
 ...
 public:
  ...
  SplineEntity(double dt) : SplineEntity(dt, 0.0){};

  SplineEntity() : SplineEntity(1.0){};
  ...
  void AddToProblem(
      ceres::Problem &problem, time_init_t times, SplineMeta &meta,
      std::vector<entity::ParameterInfo<double>> &parameters) const override {
    ...
    for (auto tt : times) {
      int i1, i2;
      double u_notused;
      segment_entity_->CalculateIndexAndInterpolationAmount(tt.first, i1,
                                                            u_notused);
      segment_entity_->CalculateIndexAndInterpolationAmount(tt.second, i2,
                                                            u_notused);
      ...
      for (int i = i1; i < (i2 + 4); ++i) {
        ...
        if (this->IsLocked()) problem.SetParameterBlockConstant(pi.data);
