...
int main(int argc, char *argv[]) {
  ...
  imu_cam_calibrator.InitSpline(calib_dataset, T_i_c_init, weight_data,
                                time_offset_imu_to_cam, gyro_bias, accl_bias,
                                telemetry_data);
  imu_cam_calibrator.InitializeGravity(telemetry_data, accl_bias);
  const double mean_reproj_err = imu_cam_calibrator.Optimize();
  LOG(INFO) << "Mean reprojection error. "<<mean_reproj_err<<"px\n";
  ...
  aligned_map<double, Eigen::Vector3d> gyro_meas = imu_cam_calibrator.GetGyroMeasurements();
  aligned_map<double, Eigen::Vector3d> accl_meas = imu_cam_calibrator.GetAcclMeasurements();
  ...
  for (auto & g : gyro_meas) {
    ...
    json_calib_results_out[t_ns_s]["gyro_imu"]["x"] = g.second[0];
    json_calib_results_out[t_ns_s]["gyro_imu"]["y"] = g.second[1];
    json_calib_results_out[t_ns_s]["gyro_imu"]["z"] = g.second[2];
  ...
  for (auto & a : accl_meas) {
    ...
    const std::string t_ns_s = std::to_string(t_ns);
    // accelerometer
    json_calib_results_out[t_ns_s]["accl_imu"]["x"] = a.second[0];
    json_calib_results_out[t_ns_s]["accl_imu"]["y"] = a.second[1];
    json_calib_results_out[t_ns_s]["accl_imu"]["z"] = a.second[2];
    // write out spline estimates
...
////            << " duration " << (end_t_ns - start_t_ns) * 1e-9 << std::endl;
  ...
  std::cout << "g: " << imu_cam_calibrator.trajectory_.getG().transpose() << std::endl;
  std::cout << "accel_bias: " << imu_cam_calibrator.trajectory_.getAccelBias().transpose()
            << std::endl;
  ...
  for (size_t i = 0; i < cam_timestamps_s.size(); ++i) {
    ...
    Sophus::SE3d spline_pose = imu_cam_calibrator.trajectory_.getPose(t_ns);
